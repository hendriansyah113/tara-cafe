...
class cetak extends Route
{
    ...
    {
        // Prepare data
        $tempPemesanan = $this->state($this->sn)->getDataTemp($kdPembelian);
        $capPembelian = strtoupper($kdPembelian);
        ...
        $qPemesanan = $this->state($this->sn)->getItemPesanan($kdPembelian);
        $tPemesanan = $this->state($this->sn)->getPembayaran($kdPembelian);
        $totalPemesanan = number_format($tPemesanan['total_final']);
        $namaPelanggan = $this->state($this->sn)->getNamaPelanggan($qPemesanan['pelanggan']);
        $waktu = date('d M Y', strtotime($qPemesanan['waktu_masuk']));

        // Insert HTML value
        $html = "<table><tr><td><img src='ladun/" . $logo . "' style='width:150px'></td>";
        $html .= "<td><span style='font-size:30px;font-family:calibri;'>Invoice Pemesanan</span><br/> " . $namaResto . ", " . $alamatResto . "</td></tr></table>";
        $html .= "<hr/>";
        $html .= "Kode Pemesanan : " . $capPembelian . "<br/>";
        $html .= "Pelanggan : " . $namaPelanggan['nama'] . "<br/>";
        $html .= "Tanggal/Waktu : " . $qPemesanan['waktu_masuk'] . "<br/>";
        $html .= "Total Pemesanan (+Tax) : Rp. " . $tPemesanan . "<br/>";

        $html .= "<div style='text-align:center'><strong>Item Pemesanan</strong></div>";
        $html .= "<table border='1' style='margin-top:15px;border-collapse:collapse;border:0px;font-size:14px;width:100%;'>";
        $html .= "<tr><th style='text-align:center'>Nama Item</th><th style='text-align:center'>Harga</th><th style='text-align:center'>Qt</th></tr>";

        // Insert data to table
        foreach ($tPemesanan as $tp) {
            $html .= "<tr><td style='padding-left:10px;'>" . $tp['nama'] . "</td><td style='padding-left:10px;'>" . $tp['harga'] . "</td><td style='padding-left:10px;'>" . $tp['jumlah'] . "</td></tr>";
        }
        ...
        // Melakukan output file Pdf
        $dompdf->stream('invoice_pemesanan.pdf', array("Attachment" => false));
    }
