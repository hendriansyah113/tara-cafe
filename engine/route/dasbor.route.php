...
class dasbor extends Route
{
    ...
    public function getDataBar()
    {
        ...
        $data['rasioProfit']        = $this->state($this->su)->getTotalPesananHarian();
        $data['transaksiHarian']    = $this->state($this->su)->getTransaksiTerakhir();
        $this->toJson($data);
    }
    ...
    {
        $this->state($this->su)->csrfCek();
        $gtt = $this->state('utilityData')->getTransaksiTerakhirList();
        foreach ($gtt as $gt) {
